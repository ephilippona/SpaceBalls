...
glm::vec3 BSpline::GetVelocity(float t)
{
    // The value of t includes where on the spline you are.
    // The integral value of t is the first control point
    // The decimal value of t is the parameter t within the segment
	...
	return vec3(GetWorldMatrix() * vec4(BSpline::GetVelocity(t - (int) t, mControlPoints[p1], mControlPoints[p2], mControlPoints[p3], mControlPoints[p4]), 1.0f));
}
...
glm::vec3 BSpline::GetVelocity(float t, const vec3& p1, const vec3& p2, const vec3& p3, const vec3& p4)
{
    // Returns the velocity of the object on the spline
    // based on parameter t and the four control points p1, p2, p3, p4
